...
public class ProfessionServiceImpl extends BaseDaoServiceImpl implements ProfessionService {
    ...
    log.info(" insert data : " + profession);
		...
		profession.setReleasedAt(currentTimeMillis);
...
return result;	
	...
	log.info(" update data : " + (profession == null ? "null" : profession.getId()));
		...
		profession.setReleasedAt(System.currentTimeMillis());
	...
	@Override
	public List<Long> getProfessionByCompanyId(Long cId, Integer start, Integer limit) throws ServiceException, ServiceDaoException {
		List<Long> pids=null;


		if (log.isInfoEnabled()){
			log.info("get cid,start,limit"+cId+","+start+","+limit);

		}
		try{
			 pids=dao.getIdList("getProfessionByCompanyId",new Object[]{ cId},start,limit,false);

		}catch (DaoException d){
		log.error("get pids is erro");
		d.printStackTrace();
	...
	@Override
	public List<Long> getProfessionByCId(Long cId, Integer start, Integer limit) throws ServiceDaoException, ServiceException {
		if(log.isInfoEnabled()){
			log.info("get id is"+cId);
		}
		if (start==null){
			start=1;
		}
		...
		List<Long> pids=null;
		try{
			pids=dao.getIdList("getProfessionByCId", new Object[]{cId},start,limit,false);
		}catch (DaoException d){
			log.error(" get ids  wrong by companyId,start,limit)  : " + cId+" , "+start+" , "+limit );
			log.error(d);
			d.printStackTrace();
			throw new ServiceDaoException(d);
		}
